...
package org.puremvc.as3.patterns.command 
{
	...
	public class MacroCommand extends Notifier implements ICommand
	{
		...
		private var subCommands:Vector.<Type>;
		 ...
		 */
		protected function addSubCommand( commandClassRef:Type ): void
		{
		...
		public final function execute( notification:INotification ) : void
		{
			while ( subCommands.length > 0) {
				var commandClassRef : Type = subCommands.shift();
				var commandInstance : ICommand = commandClassRef.getConstructor().invoke() as ICommand;
